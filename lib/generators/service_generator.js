...
class ServiceGenerator extends BaseGenerator {
  ...
  static async go({ serviceName, options = {} }) {
    ...
    let modelClassName = this.sanitizeClassName(serviceName);
    let varName = this.sanitizeVarName(serviceName);
    ...
    if (fs.existsSync(newServiceFilePath)) {
      ...
      if (serviceTemplateWithParamsReplaced == contentExistingService) {
        ...
        let canOverwrite = await this.askForOverwritePath(newServiceFilePath);
